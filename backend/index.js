...
const corsOrigin = process.env.CORS_ORIGIN
  ? [...new Set([...ALWAYS_ALLOWED, ...process.env.CORS_ORIGIN.split(',').map(o => o.trim())])]
  : [
      'http://localhost:3000',
      'http://localhost:3001',
      'http://localhost:3002',
      'http://localhost:7003',
      'https://admin.ballandboujee.com',
      'https://www.ballandboujee.com',
    ];
...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    const adminEmail = process.env.ADMIN_EMAIL || 'admin@kentaz.com';
    await sendEmail(
      adminEmail,
      `Contact Form: ${subject || 'General Inquiry'}`,
      `<p><strong>From:</strong> ${name} &lt;${email}&gt;</p><p><strong>Subject:</strong> ${subject || 'General'}</p><p>${message}</p>`
    );
...
app.post('/api/newsletter', async (req, res) => {
  try {
    ...
    // Store subscriber in User collection or just acknowledge
...
if (process.env.NODE_ENV !== 'production') {
  app.listen(PORT, () => {
    console.log(`Ball & Boujee API running on http://localhost:${PORT}`);
  });
}
