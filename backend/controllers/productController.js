...
exports.getProducts = async (req, res) => {
  try {
    const { limit = 20, page = 1, q, search, category, featured, badge, minPrice, maxPrice, sort } = req.query;
    let query = {};
    ...
    if (minPrice || maxPrice) {
      ...
      if (minPrice) query['variants.price'].$gte = parseInt(minPrice);
      if (maxPrice) query['variants.price'].$lte = parseInt(maxPrice);
    }
    ...
    else if (sort === 'price_desc') sortQuery = { 'variants.0.price': -1 };
    ...
    res.json({ success: true, products, count: products.length, total, totalPages, currentPage: pageNum });
  } catch (err) {
...
exports.getFeaturedProducts = async (req, res) => {
  try {
    const products = await Product.find({ featured: true }).limit(12).lean();
...
exports.getProduct = async (req, res) => {
  try {
    ...
    // support lookup by MongoDB _id as well
    const product = mongoose.Types.ObjectId.isValid(slug)
